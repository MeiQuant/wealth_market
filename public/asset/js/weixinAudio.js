	...
	$.fn.weixinAudio = function(options) {
		...
		Plugin.prototype = {
			init: function() {
				var self = this;
				//初始化加载音频总时长
                //$("#audio_length").text(self.getAudioDurationTime());
			...
			updateTotalTime: function() {
				var self = this,
                    ...
                    console.log('总时长:'+audioTime);
                    $("#audio_length").text(audioTime);
                	console.log('setDurationTime,Done...');
            },
			getAudioDurationTime:function(){
                var self = this,
                    time = self.Audio.duration,
                    minutes = self.getAudioMinutes(time),
                    seconds = self.getAudioSeconds(time),
